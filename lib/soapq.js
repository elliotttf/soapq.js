...
SOAPQ.prototype.request = function() {
  ...
  self.on('requestError', function(message) {
    if (retry < self.RETRY_THRESHOLD) {
      ...
      self.emit('requestErrors', 'There was a problem communicating with the server (' + self.key + ')');
    }
  });
  ...
  self.on('requestErrors', function(message) {
...
SOAPQ.prototype.respond = function() {
  ...
  var req = http.request(options, function(res) {
    ...
    res.on('end', function() {
      ...
      if (client_response.status == true) {
        console.log('Sent response to: ' + self.respondURL + ' server responded with: ' + client_response.data);
        self.remove();
        self.emit('processedRequest');
      }
      else {
        console.log('There was a problem sending the response to: ' + self.respondURL);
      }
    });
  });
  ...
  try {
    req.on('error', function(e) {
      console.log('There was a problem calling back: ' + self.respondURL + ' with the result.');
    });
    req.write(response_json);
    req.end();
  }
  catch (e) {
    console.log('There was a problem connecting to the respond server: ' + self.respondURL);
  }
};
