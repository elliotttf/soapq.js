 ...
 *
 * @param {string} apiKey
 *   The request API key that will be used to authenticate.
 * @param {string} requestKey
 *   The request requestKey that will be stored in the database.
 * @param {array} payload
 ...
 */
function SoapQ(apiKey, requestKey, payload, respondURL) {
  ...
  mongoose.connection.on('error', function(err) {
    self.emit('errorConnectingDB', err);
  });
}
 ...
 */
SoapQ.prototype.authenticate = function() {
  var self = this;
  self.Users.findOne({ 'key': self.apiKey }, function foundOne(err, doc) {
    if (err) {
      console.log(err);
...
SoapQ.prototype.save = function() {
  ...
  req.save(function onSave(err, reqSaved) {
    ...
    self.emit(
      'savedRequest',
      'Saved request to database (' + self.requestKey + ')'
    );
  });
};
...
SoapQ.prototype.remove = function() {
  var self = this;
  self.Requests.remove({ 'key': self.requestKey }, function removed(err) {
    if (err) {
      console.log(err);
    ...
    self.emit(
      'removedRequest',
      'Removed request from database (' + self.requestKey + ')'
    );
  });
};
...
SoapQ.prototype.request = function() {
  ...
  self.on('requestedOne', function requestedOne(response) {
    ...
    if (self.responses.length == self.payload.length) {
      self.emit(
        'requestedRequests',
        'All requests have been proccessed (' + self.requestKey + ')'
      );
      return;
    }
  ...
  self.on('requestError', function requestError(message) {
    if (retry < self.RETRY_THRESHOLD) {
      console.log('Retrying request (' + self.requestKey + ')');
      retry++;
      self.requestOne(payload[count]);
    }
    else {
      console.log('Too many retry failures, bailing (' + self.requestKey + ')');
      self.emit(
        'requestErrors',
...
SoapQ.prototype.requestOne = function(payload) {
    ...
    var req = http.request(options, function onRequest(res) {
      ...
      res.on('end', function onEnd() {
        ...
        if (res.statusCode != 200) {
          self.emit(
            'requestError',
            'Error during request (' + self.requestKey + ')'
          );
        }
        else {
          self.emit('requestedOne', response);
  ...
  try {
    ...
    self.emit('requestError', 'Error during request (' + self.requestKey + ')');
  }
};
  ...
  var req = http.request(options, function onRequest(res) {
    ...
    res.on('end', function onEnd() {
      ...
      if (client_response.status == true) {
        ...
        self.emit(
          'processedRequest',
          'Processed request successfully (' + self.requestKey + ')'
        );
      }
      else {
        console.log(
