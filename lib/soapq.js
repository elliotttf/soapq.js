...
SoapQ.prototype.load = function() {
  var self = this;
  self.Model.findOne({ 'key': self.key }, function foundOne(err, doc) {
    if (err) {
      console.log(err);
...
SoapQ.prototype.remove = function() {
  var self = this;
  self.Model.remove({ 'key': self.key }, function removed(err) {
    if (err) {
      console.log(err);
...
SoapQ.prototype.request = function() {
  ...
  self.on('requestError', function requestError(message) {
    if (retry < self.RETRY_THRESHOLD) {
      ...
      self.emit(
        'requestErrors',
        'There was a problem communicating with the server (' + self.key + ')'
      );
    }
  });
};
  ...
  var req = http.request(options, function onRequest(res) {
    ...
    res.on('end', function onEnd() {
      try {
        ...
        console.log('Server responded with invalid JSON, bailing (' + self.key + ')');
        return;
      }
